...
export const LinkTag: React.FC<TextEditorProps> = ({ children, value }) => {
    ...
    return (
      !value?.href?.startsWith('/') ?
        <a href={value.href} rel={rel} target={targ} aria-label={`${children}`}className="text-appVeronica100 dark:text-appRed-100 text-decoration-underline">
          {children}
        </a> : <Link href={value.href} className="text-appVeronica-100 dark:text-appRed-100 text-decoration-underline">{children}</Link>
    )
}
